...
//
// 同ディレクトリの logger.go と同じ「AWS 呼び出しの運用品質を揃える」方針の一環。
// RetryMetricsHook() を通じてリトライ層と結線できる（logger.go の
// RetryLogHook() と同じシグネチャ）。
// ヘルスチェックの判定結果と所要時間をメトリクス化しておくと、
// ALB のヘルスチェック失敗と Lambda 側の異常を切り分けやすくなる。
//
// 設計方針:
