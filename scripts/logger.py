...
Go 版ヘルスチェック（lambda_go/healthcheck）と合わせて、
このリポジトリの運用ログを 1 行 JSON に揃えるためのユーティリティ。

scripts/ 配下の検証スクリプト（validate_config.py など）から import して使う。
retry_logger() は (試行回数, 待機秒数, 例外) を受けるフック形式なので、
リトライ処理の on_retry にそのまま渡して結線できる。
